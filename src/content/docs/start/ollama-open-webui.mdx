...
Now before we install the containers, let's create some local directories to mount to our host so we can easily pass data in/out of each container. 
In later lessons we will revisit these directories.
```bash
mkdir -p ~/docker/ollama && mkdir -p ~/docker/owui
```
...
Here’s that section converted into proper Markdown:
...
![nvidia-smi.png](./attachments/nvidia-smi.png)
